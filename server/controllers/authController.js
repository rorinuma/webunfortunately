...
exports.loginUser = async (req, res) => {
  try {
    ...
    const users = await userModel.findUserCustom(userDataType, inputName, password)
    ...
    const token = jwt.sign({userId: users[0].id, username: users[0].username }, process.env.JWT_SECRET_KEY, {
      expiresIn: "7d",
    })
