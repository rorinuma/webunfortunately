...
exports.createTweet = async (userId, username, tweetId, text, image) => {
  if(tweetId) {
    await db.execute("update tweets set replies = replies + 1 where id = ?", [tweetId])
  } else tweetId = null
  console.log('posting tweet comment', tweetId)
  const query = "INSERT INTO tweets (userId, username, at, reply_to, text, image) VALUES (?, ?, ?, ?, ?, ?)";
  await db.execute(query, [userId, username, username, tweetId, text, image]);
};

exports.allTweets = async (id, limit, offset) => {
  if(id) {
    const query = "select * from tweets where reply_to = ? order by created_at desc limit ? offset ?"
    const [ tweets ] = await db.query(query, [id, limit, offset])
    return tweets
  } else {
    const query = "SELECT * FROM tweets WHERE reply_to IS NULL ORDER BY created_at DESC limit ? offset ?";
...
exports.likedTweets = async (userId, limit, offset) => {
  ...
  if(limit) {
    const query = "select likes.* from likes join users on likes.user_id = users.id where users.username = ? limit ? offset ?"
...
exports.tweetsByUsername = async (username, limit, offset) => {
  ...
  if(limit) {
    const query = "select * from tweets where username = ? order by created_at desc limit ? offset ?"
    const [ tweets ] = await db.query(query, [username, limit, offset])
    return tweets
  }
  const query = "select * from tweets where username = ? order by created_at desc"
...
exports.createReply = async (userId, tweetId, text, image) => {
  const query = "insert into tweet_replies (user_id, tweet_id, reply_text, reply_image) values (?, ?, ?, ?)"
