...
const Status = () => {
  ...
  const handleImageClick = () => {
    navigate(`/${username}/status/${statusNumber}/photo/1`, {
      state: { background: location, tweet: tweet },
    });
  };
    ...
    <section className={styles.content}>
      ...
      <article className={styles.article}>
        <div className={styles.articleLeft}>
          <div>
            <img src={pfp} alt="" className={styles.articlePfp} />
          </div>
      ...
      {tweet.image && !photoId && (
        <div className={styles.tweetImage}>
          <img src={tweet.image} alt="tweet-image" onClick={handleImageClick} />
        </div>
      )}
      <div className={styles.createdAt}>
        <div>
          <Link
            to={`/${username}/status/${statusNumber}`}
      ...
      <div className={styles.actionsContainer}>
        <IconContext.Provider value={{ size: "20px" }}>
          <div className={styles.actions}>
            <button
              className={`${styles.action} ${styles.actionBlue}`}
              onClick={() => handleReplyOnStatusComponentClick(tweet, photoId)}
            >
              <div
                className={`${styles.action} ${styles.actionImg} ${styles.blueImg}`}
              >
                <FaRegComment />
              </div>
              <div className={styles.blueAmount}>
                {tweet.replies !== 0 && tweet.replies}
              </div>
            </button>
            <button className={`${styles.action} ${styles.actionGreen}`}>
              <div
                className={`${styles.action} ${styles.actionImg} ${styles.greenImg} `}
              >
                <AiOutlineRetweet />
              </div>
            ...
            <button className={`${styles.action} ${styles.actionRed}`}>
              <div
                className={`${styles.action} ${styles.actionImg} ${styles.redImg} ${tweet.liked && styles.redFill}`}
              >
                {tweet.liked ? <FaHeart /> : <FaRegHeart />}
              </div>
              <div
                className={`${styles.redAmount} ${tweet.liked && styles.redFill}`}
              >
